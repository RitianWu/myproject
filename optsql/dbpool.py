...
class DBConnProxy:
    ...
    def __init__(self, masterconn, slaveconn):
        ...
        self._modify_methods = set(
            ['execute', 'executemany', 'last_insert_id', 'insert', 'update', 'delete'])
...
class DBPool (DBPoolBase):
    ...
    def open(self, n=1):
        ...
        newconns = []
        for i in range(0, n):
            myconn = self.connection_class[
                param['engine']](param, time.time(), 0)
            myconn.pool = self
    ...
    def run_thread():
        while True:
            ...
            try:
                ...
                log.debug("%s catch exception in acquire",
            ...
            try:
                ...
                log.debug("%s catch exception in query",
                          threading.currentThread().name)
                traceback.print_exc()
            finally:
                if conn:
